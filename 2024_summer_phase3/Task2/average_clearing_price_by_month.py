...
# 按年份和月份计算平均 demand
demand_by_month_year = df.groupby(['year', 'month'])['clearing price (CNY/MWh)'].mean().unstack(level=0)
...
plt.title('Average clearing price (CNY/MWh) by Month for Different Years')
plt.legend()
plt.grid(True)
plt.show()